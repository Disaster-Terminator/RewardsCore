...
from src.search.query_sources import LocalFileSource, BingSuggestionsSource
...
class TestQueryEngineIntegration:
    ...
    async def test_fallback_to_local_file(self, mock_config):
        ...
        with patch.object(BingSuggestionsSource, 'fetch_queries', side_effect=Exception("API Error")):
            queries = await engine.generate_queries(count=5, expand=True)
    ...
    async def test_query_deduplication_across_sources(self, mock_config):
        ...
        assert len(queries1) == len(set(q.lower() for q in queries1))
        assert len(queries2) == len(set(q.lower() for q in queries2))
