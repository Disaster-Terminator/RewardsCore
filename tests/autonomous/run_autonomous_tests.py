...
测试模式:
  ...
  --user-mode  - 用户模式：鲁棒性测试，启用防检测，3+3搜索
    ...
    parser.add_argument(
        "--integrated",
        action="store_true",
        help="运行集成测试（复用 MSRewardsApp 执行完整流程）"
    )
    ...
    parser.add_argument(
        "--output-dir",
        default="logs/test_reports",
        help="报告输出目录 (默认: logs/test_reports)"
    )
    
    parser.add_argument(
        "--cleanup",
        action="store_true",
        help="清理旧的测试数据（保留最近5次）"
    )
...
async def run_integrated_test(config_path: str = "config.yaml", user_mode: bool = False) -> Dict[str, Any]:
    """运行集成测试"""
    from tests.autonomous.integrated_test_runner import IntegratedTestRunner
