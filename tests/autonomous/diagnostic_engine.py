...
class DiagnosticEngine:
    ...
    def _get_solutions(
        ...
        for template in templates:
            ...
            solution["applicable"] = self._check_solution_applicability(
                template, context
            )
    ...
    def _cross_analyze(
        ...
        if len(issues) >= 3:
            ...
            if len(categories) >= 3:
                additional_diagnoses.append(DiagnosisResult(
    ...
    def get_critical_diagnoses(self) -> List[DiagnosisResult]:
        """获取严重诊断结果"""
        critical_categories = [
            DiagnosisCategory.ACCOUNT,
            DiagnosisCategory.AUTHENTICATION
        ]
        
        return [
            d for d in self.diagnoses
            if d.category in critical_categories or d.confidence >= 0.9
        ]
