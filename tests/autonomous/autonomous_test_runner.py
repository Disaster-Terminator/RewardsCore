...
class AutonomousTestRunner:
    ...
    async def create_context(self, storage_state: Optional[str] = None) -> bool:
        """创建浏览器上下文"""
        try:
            context_options = {
    ...
    async def execute_step(
        ...
        **kwargs
        ...
        try:
            ...
            if (self.test_config.inspection_interval > 0 and 
                self.operation_count % self.test_config.inspection_interval == 0):
            ...
            for issue in issues:
                ...
                if self.test_config.auto_screenshot:
                    await self.screenshot_manager.capture(
                        self.page, 
                        f"issue_{issue.issue_type.value}",
                        context=issue.title
                    )
    ...
    async def run_full_test_suite(self) -> Dict[str, Any]:
        ...
        try:
            ...
            results["tests"]["login"] = await self.run_test(
                "登录状态检测",
    ...
    async def _test_login_status(self, runner):
        ...
        await runner.execute_step(
            "导航到登录页面",
            lambda: runner.page.goto(login_url, wait_until="domcontentloaded")
        )
