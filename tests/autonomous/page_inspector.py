...
class PageInspector:
    ...
    def __init__(self):
        ...
        self.account_lock_indicators = [
            "account locked", "account suspended", "账号已锁定",
            "account disabled", "access denied", "访问被拒绝",
        ...
        try:
            ...
            if any(indicator in content_lower for indicator in self.login_indicators):
                if "rewards.microsoft.com" in url or "bing.com" in url:
                    login_url_patterns = [
                        "login", "signin", "oauth", "authenticate"
                    ]
    ...
    async def verify_element(
        ...
        selector: str, 
        timeout: int = 5000
    ) -> Tuple[bool, Optional[DetectedIssue]]:
        """
        验证元素是否存在
    ...
    ) -> Dict[str, Tuple[bool, Optional[DetectedIssue]]]:
        """
        批量验证元素
