...
from .page_inspector import DetectedIssue, IssueType, IssueSeverity
...
class SmartTestScenarios:
    ...
    async def scenario_login_verification(self, runner):
        ...
        if login_issues:
            ...
            has_session = Path(
                runner.config.get("account.storage_state_path", "")
            ).exists()
    ...
    async def scenario_anti_detection(self, runner):
        ...
        webdriver_check = await runner.page.evaluate(
            "() => navigator.webdriver"
        )
        ...
        user_agent = await runner.page.evaluate(
            "() => navigator.userAgent"
        )
        logger.info(f"  ℹ User-Agent: {user_agent[:50]}...")
        
        plugins = await runner.page.evaluate(
            "() => navigator.plugins.length"
        )
        logger.info(f"  ℹ 插件数量: {plugins}")
        
        languages = await runner.page.evaluate(
            "() => navigator.languages"
        )
