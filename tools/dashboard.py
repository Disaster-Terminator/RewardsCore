...
def main():
    ...
    with col1:
        ...
        delta = "已完成" if today["desktop_complete"] else f"还差 {today['target_desktop'] - today['desktop']} 次"
        st.metric(
            label=f"{status} 桌面搜索",
    ...
    with col2:
        ...
        delta = "已完成" if today["mobile_complete"] else f"还差 {today['target_mobile'] - today['mobile']} 次"
        st.metric(
            label=f"{status} 移动搜索",
    ...
    with st.expander("📈 查看趋势图表", expanded=False):
        ...
        with tab1:
            fig = go.Figure()
            fig.add_trace(go.Bar(x=df['Date'], y=df['Desktop'], name='桌面搜索', marker_color='#ff7f0e'))
            fig.add_trace(go.Bar(x=df['Date'], y=df['Mobile'], name='移动搜索', marker_color='#9467bd'))
        ...
        with tab2:
            fig = go.Figure()
            fig.add_trace(go.Bar(x=df['Date'], y=df['Gained'], name='每日获得', marker_color='#2ca02c'))
