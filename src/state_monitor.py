...
class StateMonitor:
    ...
    async def check_points_before_task(self, page) -> Optional[int]:
        """
        任务开始前检查积分
        ...
        try:
            points = await self.points_detector.get_current_points(page)
    ...
    async def check_points_after_searches(
        ...
        logger.info(f"检查积分 (已完成 {self.search_count} 次搜索)...")
        
        try:
            points = await self.points_detector.get_current_points(page, skip_navigation=True)
