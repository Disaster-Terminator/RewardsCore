...
| `source` | str | 来源（Qodo） |
| `is_pr_reviewer_guide` | bool | 是否为 PR Reviewer Guide |
| `is_code_change_summary` | bool | 是否为代码变化摘要 |
...
| 工具 | GitHub 用户名 | 特点 |
|------|--------------|------|
| **Sourcery** | `sourcery-ai bot` | 结构化摘要（Prompt for AI Agents），可通过 @ 解决 |
...
| Qodo "Review Summary by Qodo" | 代码变化摘要 | ❌ 不需要处理 |
| Qodo "PR Reviewer Guide 🔍" | 改进意见摘要 | ✅ 需要处理 |
